...
def vlcLogin(hostname, pwdFile):                #参数(主机名,字典文件)
    try:
        with open(pwdFile, 'r') as pf:          #打开字典文件
            for line in pf.readlines():         #循环字典文件中的每一行
                time.sleep(1)                   #等待一秒
                userName = line.split(':')[0]   #从读取的内容中取出用户名
                passWord = line.split(':')[1].strip('\r').strip('\n')  #从读取的内容中取出密码
                print('[+] Trying:'+ userName + ':' + passWord)
                try:
                    with FTP(hostname) as ftp:  #以主机名为参数构建Ftp对象
                    ...
                    #产生异常表示没有登陆成功，这里我们不用管它，继续尝试其他用户名和密码
                    pass
    except IOError as e:
...
def main():
    ...
    #添加-H命令dest 可以理解为咱们解析时获取-H参数后面的变量名，jelp是这个命令的帮助信息
    parser.add_argument('-H',dest='hostName',help='The host list with "," space')
    parser.add_argument('-f',dest='pwdFile',help='Password dictonary file')
    options = None
    try :
